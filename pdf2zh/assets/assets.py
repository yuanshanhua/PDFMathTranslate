...
def _retry_if_not_cancelled_and_failed(retry_state):
    ...
    if retry_state.outcome.failed:
        ...
        if isinstance(exception, httpx.HTTPError | ConnectionError | ValueError | TimeoutError):
...
async def get_font_metadata(client: httpx.AsyncClient | None = None, upstream: str = None):
...
async def get_fastest_upstream_for_font(client: httpx.AsyncClient | None = None, exclude_upstream: list[str] = None):
...
async def get_doclayout_onnx_model_path_async(client: httpx.AsyncClient | None = None):
    onnx_path = get_cache_file_path("doclayout_yolo_docstructbench_imgsz1024.onnx", "models")
    ...
    await download_file(client, url, onnx_path, DOCLAYOUT_YOLO_DOCSTRUCTBENCH_IMGSZ1024ONNX_SHA3_256)
...
async def get_table_detection_rapidocr_model_path_async(
    ...
    logger.info(f"Download table detection rapidocr model from {fastest_upstream} success")
    return onnx_path
...
async def get_font_and_metadata_async(
    ...
    if "sha3_256" not in font_metadata[font_file_name]:
        logger.critical(f"Font {font_file_name} not found in {font_metadata}")
        exit(1)
    await download_file(client, url, cache_file_path, font_metadata[font_file_name]["sha3_256"])
    return cache_file_path, font_metadata[font_file_name]
...
def get_font_family(lang_code: str):
    font_family = get_font_family_(lang_code)
    return font_family
...
async def download_all_fonts_async(client: httpx.AsyncClient | None = None):
    ...
    font_tasks = [
        asyncio.create_task(get_font_and_metadata_async(font_file_name, client, fastest_upstream, font_metadata))
        for font_file_name in EMBEDDING_FONT_METADATA
    ]
...
async def async_warmup():
    ...
    async with httpx.AsyncClient() as client:
        ...
        onnx_task2 = asyncio.create_task(get_table_detection_rapidocr_model_path_async(client))
...
async def generate_offline_assets_package_async(output_directory: Path | None = None):
    ...
    if output_directory is None:
        output_path = get_cache_file_path(f"offline_assets_{offline_assets_tag}.zip", "assets")
    else:
...
async def restore_offline_assets_package_async(input_path: Path | None = None):
    ...
    if input_path is None:
        input_path = get_cache_file_path(f"offline_assets_{offline_assets_tag}.zip", "assets")
    else:
        ...
        offline_assets_tag_from_input_path = re.match(r"offline_assets_(.*)\.zip", input_path.name).group(1)
        ...
        for file_type, file_descs in file_list.items():
            for file_desc in file_descs:
                ...
                if not verify_file(file_path, file_desc["sha3_256"]):
                    logger.critical("Offline assets package is corrupted, please delete it and try again")
                    exit(1)
